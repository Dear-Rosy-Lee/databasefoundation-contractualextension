...
namespace YuLinTu.Component.XiZangLZ
{
    ...
    /// 填写地块相关信息
    /// </summary>
    public partial class ContractWarrantPrinter
    {
        /// <summary>
        /// 设置地块信息
        /// </summary>
        ...
        protected virtual void WriteFamilyLandCalInformation(List<ContractLand> landCollection)
        {
            ...
            List<ContractLand> totalLands = landCollection.FindAll(ld => ld.ConstructMode == ((int)eConstructMode.Family).ToString()); //家庭承包方式
            double conLandActualArea = 0.0;
            double conLandawareArea = 0.0;
            totalLands.ForEach(l =>
            {
                conLandActualArea += l.ActualArea;
                conLandawareArea += l.AwareArea;
                // 去除四至中的括号
            });
            SetBookmarkValue("bmLandCount", totalLands.Count < 1 ? "  " : totalLands.Count.ToString());
            SetBookmarkValue("bmLandArea", conLandawareArea.ToString("0.00"));

            var lands = totalLands.Take(BookLandNum.Value);
            ...
            //for (int i = 3; i < pageSize; i += 2)
            //{
            //    AddSection();
            //}

            InitalizeLandImageInformation(lands.Take(PAGECOUNT1).ToList(), 0, 0, 2);

            for (int i = 0; i < pageSize - 1; i++)
            {
                InitalizeLandImageInformation(
                    lands.Skip(PAGECOUNT1 + (i) * PAGECOUNT2).Take(PAGECOUNT2).ToList(), (i / 2) + 1, (i + 1) % 2);
            }
            ...
            //写入扩展页地块
            if (totalLands.Count > BookLandNum.Value)
            {
                totalLands.RemoveRange(0, BookLandNum.Value);
                if (!UseExcel)
                {
                    AgriLandExporeeProgress(totalLands);
                }
                else
                {
                    AgriLandExporessExcelProgress(totalLands);
                }
            }
        }

        private void InitalizeLandImageInformation(List<ContractLand> lands, int sectionIndex, int tableIndex, int startRow = 1)
        {
            ...
            {
                SetTableCellValue(sectionIndex, tableIndex, row, 0, land.CadastralNumber.IsNullOrBlank() ? "" : land.CadastralNumber);
                SetTableCellValue(sectionIndex, tableIndex, row, 1, GetNeighbor(land));
                SetTableCellValue(sectionIndex, tableIndex, row, 2, land.AwareArea.ToString("0.00"));
                SetTableCellValue(sectionIndex, tableIndex, row, 3, land.IsFarmerLand == null ? "" : (land.IsFarmerLand.Value ? "是" : "否"));
                SetTableCellValue(sectionIndex, tableIndex, row, 4, land.Comment.IsNullOrBlank() ? "" : land.Comment);
                row++;
            }
        }

        private string GetNeighbor(ContractLand land)
        {
