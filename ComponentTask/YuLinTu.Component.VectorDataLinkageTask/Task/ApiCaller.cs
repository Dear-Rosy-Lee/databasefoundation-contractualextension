...
namespace YuLinTu.Component.VectorDataLinkageTask
{
    public class ApiCaller
    {
        ...
        /// <returns></returns>
        public string PostDataAsync(string url, string jsonData, string app_id, string app_key)
        {
            try
            {
                // 将JSON数据转换为HttpContent
                ...
                {
                    client.DefaultRequestHeaders.Add("t-open-api-app-id", app_id);
                }
                if (!string.IsNullOrEmpty(app_key))
                {
                    client.DefaultRequestHeaders.Add("t-open-api-app-key", app_key);
                }
                // 发送 POST 请求
                HttpResponseMessage response = client.PostAsync(url, content).GetAwaiter().GetResult();
                //response.EnsureSuccessStatusCode();
