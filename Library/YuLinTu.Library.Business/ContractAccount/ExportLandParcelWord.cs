...
namespace YuLinTu.Library.Business
{
    ...
    ///  农村土地承包经营权地块示意图
    /// </summary>
    public class ExportLandParcelWord : AgricultureWordBook
    {
        #region Fields
        ...
        private int fromTwoPageTableCount;//从第二页开始的表个数，包括第二页
       ...
       // public IDbContext DbContext { get; set; }
        ...
        /// 构造函数
        /// </summary>
        public ExportLandParcelWord(IDbContext db)
        {
        ...
        private void InitalizeLandRowInformation()
        {
            ...
            if (geoLandCollection != null && geoLandCollection.Count <= 8)
            {
                SetTableCellValue(0, 8, 2, "1-1");
                DeleteTable(1);
                DeleteParagraph();
                return;
            }

            int pagecount = geoLandCollection.Count / 8;
            int totalPage = geoLandCollection.Count % 8 == 0 ? pagecount : (pagecount + 1);

            if (totalPage >= 2) fromTwoPageTableCount = totalPage - 1;

            //从第二页后添加8的表格
            if (fromTwoPageTableCount >= 1)
            {
                for (int i = 0; i < fromTwoPageTableCount; i++)
                {
                    AddTable(0, i, false);
                }
            }
            SetTableCellValue(0, 8, 2, "1-" + totalPage.ToString());
            for (int index = 1; index <= fromTwoPageTableCount; index++)
            {
                SetTableCellValue(index, 8, 2, (index + 1).ToString() + "-" + totalPage.ToString());
            }
        }
        ...
        private void InitalizeLandBookMarkInformation()
        {
            ...
            {
                for (int colInex = 2; colInex < 6; colInex++)
                {
        ...
        {
            if (geoLandCollection.Count > 8)
            {
                geoLandCollection.RemoveRange(0, 8);
            }
            else
            {
                return;
            ...
            for (int index = 0; index < fromTwoPageTableCount; index++)
            {
                for (int rowIndex = 2; rowIndex < 4; rowIndex++)
                {
                    for (int colInex = 2; colInex < 6; colInex++)
                    {
                        ...
                        ContractLand land = geoLandCollection[landIndex];
                        InsertImageShape(land, rowIndex, colInex, index + 1);
                        landIndex++;
                    }
                }
            }
        ...
        private void InitalizeAllEngleView()
        {
            ...
            if (System.IO.File.Exists(fileName))
            {
                ...
                {
                    InsertImageCellWithoutPading(AgricultureBookMark.AgricultureAllShape, fileName, 200, 170);
                }
                else
                {
        ...
        public void GetAllMultiParcel()
        {
            try
            {
                ...
                Application.Current.Dispatcher.Invoke(new Action(() =>
                {
                    ...
                    {
                        MessageBox.Show("数据中存在非闭合的线！", "错误");
                        YuLinTu.Library.Log.Log.WriteException(this, "导出农村土地承包经营权地块示意图", e.Message + e.StackTrace);
                        return;
                    }
                }));
        ...
        public void GetNeighorParcels()
        {
            ...
            var visibleBounds = new CglEnvelope(0, 0, mapW, mapH + 10);
            try
            {
                MapShapeUI map = null;
                ...
                if (SettingDefine.ShowlandneighborLabel)
                {
                    ...
                    foreach (var geoLand in geoLandCollection)
                    {
                        ...
                        var geolandbuffershape = shapeextend.Buffer(SettingDefine.Neighborlandbufferdistence);
                        var tempLands = landStation.GetIntersectLands(geoLand, geolandbuffershape);
                        queryLandList.AddRange(tempLands);
                        if (snc.CurrentZoneQueryLandList.Any(fdsa => fdsa.LandNumber == geoLand.LandNumber) == false)
                        {
                            snc.CurrentZoneQueryLandList.Add(geoLand);
                        }
                    }
                    ...
                    neiborghinfodics = snc.InitializeNeighborInfo(snc.CurrentZoneQueryLandList, queryResultStr);
                }
                ...
                foreach (var geoLand in geoLandCollection)
                {
                    if (x < LstViewOfNeighorParcels.Count)
                    {
                    ...
                    Application.Current.Dispatcher.Invoke(new Action(() =>
                    {
                        ...
                        ViewOfNeighorParcels.Paper.Model.Height = visibleBounds.Height + 10;// 136;
                    ...
                    List<ContractLand> tempLands = null;
                    //var tempLands = landStation.GetIntersectLands(geoLand, geolandbuffershape);
                    ...
                    {
                        tempLands = GetIntersectLands(geoLand, geolandbuffershape);
                    }
                    else
                    {
                        ...
                        if (SettingDefine.IsShowJZDNumber)
                        {
                            if (exportLandParcelMainOperation.FilterjzdNodes != null)
                            {
                                foreach (var tempitem in exportLandParcelMainOperation.FilterjzdNodes)
                                {
                                    ...
                                    //if (temitemdot == null) continue;
                                    var element = exportLandParcelMainOperation.AddNeiberJzdNumberlabel(ViewOfNeighorParcels, diagram, env, temitemdot, geoLand, index++);  //添加界址点号标注
                                    if (element == null)
                                    {
                                        index--;
                                        continue;
            ...
            {
                Log.Log.WriteError(this, "ExportContractLandParcelWord", ex.ToString());
                throw new YltException("获取邻地块示意图失败!");
            }
        }
        ...
        private DynamicGeoSource GetSetALLOwnerlands(List<FeatureObject> listOwenrFeature, MapShapeUI map, int layerCountIndex)
        {
            ...
            Application.Current.Dispatcher.Invoke(new Action(() =>
            {
                ...
                if (lyerOwnerExtent != null)
                {
                    ...
                    ((lyerOwnerExtent.Renderer as SimpleRenderer).Symbol as SimplePolygonSymbol).BorderStrokeColor = SettingDefine.ViewOfAllvpLandColor;
