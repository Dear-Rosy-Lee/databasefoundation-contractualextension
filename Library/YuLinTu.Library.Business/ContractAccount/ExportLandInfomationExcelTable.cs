...
namespace YuLinTu.Library.Business
{
    ...
    public class ExportLandInfomationExcelTable : ExportExcelBase
    {
        ...
        private bool BeginWrite()
        {
            ...
            WriteTempLate();
            SetLineType("A5", "R" + (index - 1), false);
        ...
        private void WriteCurrentZoneInformation(ContractLand land, int index)
        {
            ...
            InitalizeRangeValue("O" + index, "O" + index, land.NeighborNorth != null ? land.NeighborNorth : "/");
            InitalizeRangeValue("P" + index, "P" + index, land.AwareArea == 0 ? SystemDefine.InitalizeAreaString() : ToolMath.SetNumbericFormat(land.AwareArea.ToString(), 2));
            InitalizeRangeValue("Q" + index, "Q" + index, land.ActualArea == 0 ? SystemDefine.InitalizeAreaString() : ToolMath.SetNumbericFormat(land.ActualArea.ToString(), 2));
            InitalizeRangeValue("R" + index, "R" + index, land.Comment);
        }
        ...
        {
            string title = GetRangeToValue("A1", "S1").ToString();
            //title = SystemDefine.GetTableHeaderStr(CurrentZone) + title;
            InitalizeRangeValue("A" + 1, "R" + 1, title);
            InitalizeRangeValue("A" + 2, "F" + 2, string.Format("发包方名称:{0}", SenderName));
        ...
        private void WriteCount()
        {
            ...
            InitalizeRangeValue("N" + index, "N" + index, "\\");
            InitalizeRangeValue("O" + index, "O" + index, "\\");
            InitalizeRangeValue("P" + index, "P" + index, AwareArea > 0 ? ToolMath.SetNumbericFormat(AwareArea.ToString(), 2) : "\\");
            InitalizeRangeValue("Q" + index, "Q" + index, ActualArea > 0 ? ToolMath.SetNumbericFormat(ActualArea.ToString(), 2) : "\\");
