...
namespace YuLinTu.Component.VectorDataTreatTask
{
    public class ApiCaller
    {
        ...
        /// <returns></returns>
        public string PostDataAsync(string url, string jsonData, string aid, string akey, string szdy)
        {
            try
            {
                // 将JSON数据转换为HttpContent
                HttpContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
                client.DefaultRequestHeaders.Add("t-open-api-app-id", aid);
                client.DefaultRequestHeaders.Add("t-open-api-app-key", akey);
