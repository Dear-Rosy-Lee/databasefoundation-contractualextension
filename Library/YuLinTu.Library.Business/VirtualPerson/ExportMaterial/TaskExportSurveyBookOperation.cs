...
namespace YuLinTu.Library.Business
{
    ...
    /// 导出测绘申请书任务类
    /// </summary>
    public class TaskExportSurveyBookOperation : Task
    {
        #region Ctor
        ...
        /// 构造函数
        /// </summary>
        public TaskExportSurveyBookOperation()
        { }
        ...
        {
            TaskExportSurveyBookArgument argument = Argument as TaskExportSurveyBookArgument;
            if (argument == null)
            {
                return;
            }
            ...
            try
            {
                ...
                bool canOpen = ExportSurveyBookWord(argument, listPerson);
                if (canOpen)
                    CanOpenResult = true;
            }
            catch (Exception ex)
            {
                YuLinTu.Library.Log.Log.WriteException(this, "TaskExportSurveyBookOperation(导出测绘申请书任务)", ex.Message + ex.StackTrace);
                this.ReportException(ex, "导出测绘申请书出现异常!");
            }
        }
        ...
        /// 导出数据到测绘申请书
        /// </summary>
        public bool ExportSurveyBookWord(TaskExportSurveyBookArgument argument, List<VirtualPerson> listPerson)
        {
            bool result = false;
            try
            {
                ...
                foreach (VirtualPerson family in listPerson)
                {
                    ...
                    export.Date = argument.Time == null ? DateTime.Now : argument.Time;
                    export.VirtualPerson = family;
                    export.CurrentZone = argument.CurrentZone;
                    export.ZoneList = zoneList == null ? new List<Zone>() : zoneList;
                    export.OpenTemplate(templatePath);
                    export.SaveAs(family, openFilePath + @"\" + familyNuber + "-" + family.Name + "-" + TemplateFile.VirtualPersonSurveyBook + ".doc");
                    this.ReportProgress((int)(1 + vpPercent * index), markDesc + family.Name);
                    index++;
                }
                result = true;
                string info = string.Format("{0}导出{1}张测绘申请书", markDesc, index - 1);
            ...
            {
                result = false;
                this.ReportError("导出测绘申请书失败");
                YuLinTu.Library.Log.Log.WriteException(this, "ExportSurveyBookWord(测绘申请书处理)", ex.Message + ex.StackTrace);
            }
            return result;
        }
