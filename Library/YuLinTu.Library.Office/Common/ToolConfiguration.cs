...
namespace YuLinTu.Library.Office
{
    ...
    /// 配置文件
    /// </summary>
    public class ToolConfigurationSetting
    {
        /// <summary>
        /// 判断配置节是否存在
        /// </summary>
        ...
        public static string GetDefaultPath()
        {
            ...
            filepath = Path.Combine(filepath, "YuLinTu Files\\YuLinTu.Library.Business.SystemSetDefine.xml");
            XmlDocument xml = new XmlDocument();
            string defaultpath = "";
            try
            {
                xml.Load(filepath);
                XmlNode root = xml.SelectSingleNode("//DefaultPath");
                if (root != null)
                    defaultpath = root.InnerText;
            }
            catch
            {
