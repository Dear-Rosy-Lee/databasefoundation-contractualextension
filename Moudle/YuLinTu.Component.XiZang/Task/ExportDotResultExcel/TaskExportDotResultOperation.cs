...
namespace YuLinTu.Component.XiZangLZ
{
    ...
    public class TaskExportDotResultOperation : Task
    {
        ...
        protected override void OnGo()
        {
            ...
            bool canOpen = ExportDotResultExcel(metadata, fileName, listGeoLand, currentPerson,zone, entireZones);
            if (canOpen)
            {
                CanOpenResult = true;
            }
        ...
        private bool ExportDotResultExcel(TaskExportDotResultArgument argument,string fileName, List<ContractLand> listGeoLand, 
            VirtualPerson currentPerson,Zone currentZone, List<Zone> entireZones)
        {
            bool canOpen = true;
            ...
            var openFilePath = argument.FileName;
            foreach (var geoLand in listGeoLand)
            {
                ...
                export.DotCollection =dotStation.GetByLandID(geoLand.ID);
                export.LineCollection=coilStation.GetByLandID(geoLand.ID);
                export.CurrentZone = currentZone;
                export.CurrentLand = geoLand;
                string filePath = openFilePath + @"\" + geoLand.LandNumber + "-" + "西藏农村土地承包经营权界址点成果表";
