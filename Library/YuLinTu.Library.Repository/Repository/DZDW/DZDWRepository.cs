...
namespace YuLinTu.Library.Repository
{
    ...
    /// 集体建设用地点状地物的数据访问类
    /// </summary>
    public class DZDWRepository : RepositoryDbContext<DZDW>, IDZDWRepository
    {
          #region Ctor

        private IDataSourceSchema m_DSSchema = null;

        public DZDWRepository(IDataSource ds)
            : base(ds)
        {
        ...
        private bool CheckTableExist()
        {
            //try
            //{
            //    return m_DSSchema.AnyElement(m_DSSchema.GetElements()[0].Schema, typeof(DZDW).GetAttribute<DataTableAttribute>().TableName);
            //}
            //catch (Exception)
            //{
            //    return false;
        ...
        public List<DZDW> GetByZoneCode(string zoneCode)
        {
            ...
            object entity = (from q in DataSource.CreateQuery<DZDW>()
                             where q.ZoneCode.Equals(zoneCode)
                             orderby q.BSM
                             select q).ToList();
            return entity as List<DZDW>;
        }
