...
namespace YuLinTu.Component.NingMing
{
    ...
    public partial class ExportContractorSurveyExcel
    {
        ...
        private void WriteTablePerson(Person person, int pindex)
        {
            ...
            {
                columnIndex++;
                SetRange(PublicityConfirmDefine.GetColumnValue(columnIndex) + pindex, PublicityConfirmDefine.GetColumnValue(columnIndex) + pindex, person.ExtensionPackageNumber);//成员延包土地份数
                int count = 0;
                int.TryParse(person.ExtensionPackageNumber, out count);
                packageCount += count;
            }
        ...
        private int WriteContractLand(List<ContractLand> cs, int hight, int cindexx, string telephone)
        {
            ...
            foreach (ContractLand land in cs)
            {
                ...
                {
                    columnIndex++;
                    SetRange(PublicityConfirmDefine.GetColumnValue(columnIndex) + tempIndex, PublicityConfirmDefine.GetColumnValue(columnIndex) + (tempIndex + hight - 1), tempContractDelayCount > 0.0 ? ToolMath.SetNumbericFormat(tempContractDelayCount.ToString(), 2) : SystemSet.InitalizeAreaString());//实测总面积
                }
