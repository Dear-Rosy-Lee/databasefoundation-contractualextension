...
namespace YuLinTu.Library.Business
{
    ...
    /// 导出非家庭承包单户申请书
    /// </summary>
   public class TaskRequireBookByOtherOperation: Task
    {
        #region Fields
        ...
        /// 构造函数
        /// </summary>
        public TaskRequireBookByOtherOperation()
        {
        }
        ...
        {
            TaskRequireBookByOtherArgument metadata = Argument as TaskRequireBookByOtherArgument;
            if (metadata == null)
            {
                return;
            }
            ...
            {
                //导出单户申请书-单个或多个
                BatchExportApplicationByOther(zone, SelectContractor, parent, fileName, business);
            }
        ...
        private void BatchExportApplicationByOther(Zone currentZone, List<Zone> childrenZone, Zone parentZone, string savePath, ConcordBusiness business)
        {
            ...
            List<VirtualPerson> vps = business.GetByZone(currentZone.FullCode);
                ...
                {
                    string message = null;
                    bool flag = business.ExportApplicationByOtherBookWord(currentZone, path, vp, out message);
                    if (flag == true)
                    {
                        returnValue = true;
                ...
                string info = index == 1 ? string.Format("{0}无其他承包方式合同数据", currentZone.FullName) : string.Format("{0}下成功导出{1}其他承包方式单户申请书", currentZone.FullName, index - 1);
                  if (vps.Count == 0)
            {
        ...
        /// 多个导出单户申请书
        /// </summary>
        private void BatchExportApplicationByOther(Zone currentZone, List<VirtualPerson> vps, Zone parentZone, string savePath, ConcordBusiness business)
        {
            TaskRequireBookByOtherArgument metadata = Argument as TaskRequireBookByOtherArgument;
            ...
            {
                string message = null;
                bool flag = business.ExportApplicationByOtherBookWord(currentZone, savePath, vp, out message);
                if (flag == true)
                {
                    returnValue = true;
            ...
            string info = index == 1 ? string.Format("{0}无非家庭承包方式合同书数据", currentZone.FullName) : string.Format("{0}下成功导出{1}个非家庭承包方式单户申请书", currentZone.FullName, index - 1);
            if (vps.Count == 0)
            {
                this.ReportWarn(info);
                this.ReportProgress(100, "完成");
            ...
            this.ReportProgress(100, "完成");
            this.ReportInfomation(string.Format("共导出{0}非家庭承包方式单户申请书", index - 1));
        }
