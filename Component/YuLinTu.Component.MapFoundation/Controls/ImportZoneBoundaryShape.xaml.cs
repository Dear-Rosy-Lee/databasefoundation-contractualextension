...
namespace YuLinTu.Component.MapFoundation
{
    ...
    /// 导入区域界线图斑界面
    /// </summary>
    public partial class ImportZoneBoundaryShape : InfoPageBase
    {
        #region Ctor
        ...
        /// 构造函数
        /// </summary>
        public ImportZoneBoundaryShape(IWorkpage page, string desc = "")
        {
            InitializeComponent();
            DataContext = this;
            this.Workpage = page;
        ...
        private ImportZoneBoundaryDefine config;
        private ImportZoneBoundaryDefine importZbDefine;
        ...
        /// 界址点图斑实体配置
        /// </summary>
        public ImportZoneBoundaryDefine ImportZbDefine
        {
            get { return importZbDefine; }
            set
            {
                importZbDefine = value;
                this.ProGrid.Object = ImportZbDefine;
            }
        }
        ...
        /// 读取的区域界线图斑所有字段名称
        /// </summary>
        public List<KeyValue<int, string>> ZbNameList { get; set; }
        ...
        private void OnLoadConfig(List<string> zbColumnNameList)
        {
            CommonConfigSelector.DisplayValueList = zbColumnNameList;
            ZbNameList = CommonConfigSelector.GetUserConfigColumnInfo(zbColumnNameList.Count); //获取定义的数据源
            ProGrid.EnableObjectMetadataCache = false;
            ProGrid.Properties["index"] = ZbNameList;
            ProGrid.Alert += ProGrid_Alert;
            ProGrid.Properties["importZoneBoundaryShape"] = FileName;
            systemCenter = TheApp.Current.GetSystemSettingsProfileCenter();  //系统配置
            var profile = systemCenter.GetProfile<ImportZoneBoundaryDefine>();  //得到section部分 此方法已经判断了section为空的情况，为空就用默认构造
            var section = profile.GetSection<ImportZoneBoundaryDefine>();
            config = (section.Settings as ImportZoneBoundaryDefine);   //得到经反序列化后的对象
            var configCopy = config.Clone();
            var keylist = configCopy.DictProperties();
            foreach (var item in keylist)
            {
                if (!zbColumnNameList.Contains(item.Value.ToString()))
                {
                    configCopy.SetPropertyValue(item.Key, "None");
                }
            }
            ImportZbDefine = configCopy as ImportZoneBoundaryDefine;
        }
        ...
        private void btnFileSelect_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                List<string> zbColumnNameList = new List<string>();
                ss.ForEach(t => zbColumnNameList.Add(t.ColumnName));
                CommonConfigSelector.DisplayValueList = null;
                OnLoadConfig(zbColumnNameList);
            }
            catch
            {
        ...
        private void btnExcuteImport_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                config.CopyPropertiesFrom(ImportZbDefine);
                systemCenter.Save<ImportZoneBoundaryDefine>();
            }));
        ...
        private void allCheck_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                PropertyInfo[] infos = typeof(ImportZoneBoundaryDefine).GetProperties();
                ...
                for (int i = 0; i < infos.Length; i++)
                {
                    ...
                    {
                        info.SetValue(ImportZbDefine, "None", null);
                        continue;
                    }
                    if (display == null)
                        continue;
                    ...
                    if (fvalue)
                    {
                        info.SetValue(ImportZbDefine, display.Name, null);
                    }
                }
            }
            catch
