...
namespace YuLinTu.Component.XiZangLZ
{
    ...
    /// 导出登记调查确认书任务类
    /// </summary>
    public class TaskExportConfirBookOperation : Task
    {
        #region Ctor
        ...
        /// 构造函数
        /// </summary>
        public TaskExportConfirBookOperation()
        { }
        ...
        {
            TaskExportConfirBookArgument argument = Argument as TaskExportConfirBookArgument;
            if (argument == null)
            {
                return;
            }
            ...
            try
            {
                ...
                bool canOpen = ExportFamilyGSBExcel(argument, listPerson, listLand, listDict, tissue, concords);
                if (canOpen)
                    CanOpenResult = true;
            }
            catch (Exception ex)
            {
                YuLinTu.Library.Log.Log.WriteException(this, "TaskExportPublishWordOperation(导出登记调查确认书任务)", ex.Message + ex.StackTrace);
                this.ReportException(ex, "导出登记调查确认书出现异常!");
            }
        }
        ...
        /// 导出登记调查确认书
        /// </summary>
        public bool ExportFamilyGSBExcel(TaskExportConfirBookArgument argument, List<VirtualPerson> listPerson, List<ContractLand> listLand,
            List<Dictionary> listDict, CollectivityTissue tissue, List<ContractConcord> concords)
        {
            ...
            try
            {
                ...
                foreach (VirtualPerson vp in listPerson)
                {
                    ...
                    string tempPath = TemplateHelper.ExcelTemplate("西藏农村土地承包经营权登记调查确认书");
                    var export = new ExportFamilySurveyBook(tempPath, dictoryName);
                    ...
                    export.FilePath = argument.FileName;
                    ...
                    string fileName = openFilePath + @"\" + vp.Name + "-" + "西藏农村土地承包经营权登记调查确认书";
                    export.Write();
                ...
                result = true;
                string info = string.Format("{0}导出{1}户承包方,共{2}个登记调查确认书", markDesc, listPerson.Count, indexOfVp - 1);
            ...
            {
                result = false;
                this.ReportError("导出登记调查确认书失败");
                YuLinTu.Library.Log.Log.WriteException(this, "ExportLandWord(导出登记调查确认书)", ex.Message + ex.StackTrace);
            }
            return result;
        }
