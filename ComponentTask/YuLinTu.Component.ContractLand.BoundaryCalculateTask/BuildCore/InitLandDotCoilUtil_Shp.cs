...
namespace YuLinTu.Component.ContractedLand.BoundaryCalculateTask
{
    ...
    class InitLandDotCoilUtil_Shp
    {
        ...
        //        //if(CglHelper.equal(jzd.X,448724.333679,0.001)
        //        //    && CglHelper.equal(jzd.Y, 3483816.152527, 0.001))
        //        //{
        ...
        private static void parsePoints(Coordinate[] coords, ShortZd_cbd cbd, bool fShell)
        {
            ...
            for (int i = 0; i < coords.Length; ++i)
            {
                ...
                {
                    if (!CglHelper.isSamePoint(preJzd, jzd))
                    {
        ...
        {
            string str = null;
            FileHelper.EnumFiles(sSpatialReferencesPath, fi =>
            {
