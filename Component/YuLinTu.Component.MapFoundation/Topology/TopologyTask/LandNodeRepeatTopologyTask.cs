...
{
    public class LandNodeRepeatTopologyTask : LandTopologyTask
    {
        #region Ctor

        public LandNodeRepeatTopologyTask()
        {
            Name = "面要素节点重复";
            Description = "检查同一面状要素内节点间实地距离应大于容差距离";
        }
        ...
        private TopologyError BuildError(double x1, double y1, double x2, double y2, double len2)
        {
            var args = Argument as LandTopologyTaskArgument;
            ...
            error.Description = string.Format("面要素节点重复");
