...
namespace YuLinTu.Component.StockRightBase.Bussiness
{
    public partial class ReadExcelBase : ExcelBase
    {
        ...
        private void ResetInfo()
        {
            ...
            Dictionary<string, Guid> landNumberList = new Dictionary<string, Guid>();

            foreach (var entity in excelData)
            {
                var cardId = entity.Contractor.Number;
                var contractor = _bussnessObject.VirtualPersonsAll?.FirstOrDefault(vp => vp.Number == cardId);
                if (contractor != null)
                {
                    entity.Contractor.ID = contractor.ID;
                }
                else
                {
                    entity.Contractor.ID = Guid.NewGuid();
                }

                if (entity.LandList != null && entity.LandList.Count != 0)
                {
                    foreach (var land in entity.LandList)
                    {
                        if (!landNumberList.ContainsKey(land.LandNumber))
                        {
                            var landTemp = _bussnessObject.ContractLands.FirstOrDefault(l => l.LandNumber == land.LandNumber);
                            var guid = landTemp != null ? landTemp.ID : Guid.NewGuid();
                            land.ID = guid;

                            landNumberList.Add(land.LandNumber, guid);
                        }
                        else
                        {
                            land.ID = landNumberList[land.LandNumber];
                        }
                    }
                }
            }
        ...
        private Dictionary<string, Guid> GetLandIdDic()
        {
            ...
            if (_allItem != null && _allItem.Length != 0)
            {
                for (int i = 0; i < _rangeCount&&i<_breakIndex; i++)
                {
                    ...
                    {
                        landIdDic.Add(landNumberNoWhite, new Guid());
                    }
                }
            }
