...
{
    /// <summary>
    /// ImportCBDShapeByRightMenu.xaml 的交互逻辑-右键导入地块空间数据
    /// </summary>
    public partial class ImportCBDShapeByRightMenu : InfoPageBase
    {
        #region Fields
        ...
        /// </summary>
        public CommonSelectZoneDefine CommonBusinessSettingDefine
        {
            get { return (CommonSelectZoneDefine)GetValue(CommonBusinessDefineProperty); }
            set { SetValue(CommonBusinessDefineProperty, value); }
        }

        public static readonly DependencyProperty CommonBusinessDefineProperty =
            DependencyProperty.Register("CommonBusinessSettingDefine", typeof(CommonSelectZoneDefine), typeof(ImportCBDShapeByRightMenu));
        ...
        #region Ctor
        public ImportCBDShapeByRightMenu(IWorkpage page, string header, Zone currentZone, string desc = "")
        {
            InitializeComponent();
            this.Header = header;
            this.Workpage = page;
            ...
            CommonBusinessSettingDefine = CommonSelectZoneDefine.GetIntence();
            if (currentZone != null)
            {
                CommonBusinessSettingDefine.CurrentZoneFullName = currentZone.FullName;
                CommonBusinessSettingDefine.CurrentZoneFullCode = currentZone.FullCode;
            }
        }
        ...
        private void btnFileSelect_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    message += ",当前数据库坐标为: " + dbpi.Name + "(" + dbReference.WKID + ")";
                }
                if (dbReference.IsGEOGCS() || !dbReference.IsValid())
                {
                    message += ",当前数据库坐标为: " + dbpi.GeographicInfo.Name + "(" + dbReference.WKID + ")";
                }
            }

            if (shpReference != null && !shpReference.Equals(dbReference))
            {
            ...
            catch
            {
                ...
                {
                    Header = "提示",
                    Message = "当前打开Shape文件有误，请检查文件是否正确",
        ...
        private void btnChangeCommon_Click(object sender, RoutedEventArgs e)
        {
            ...
            zoneSelectorPanel.SelectorZone = new ZoneDataItem() { FullCode = SelectZone != null ? SelectZone.FullCode : CommonBusinessSettingDefine.CurrentZoneFullCode };
