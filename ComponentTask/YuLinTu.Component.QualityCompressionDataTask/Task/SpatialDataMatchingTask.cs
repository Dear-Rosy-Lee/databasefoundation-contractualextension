    ...
    public class SpatialDataMatchingTask : Task
    {
        ...
        private bool DataCheck()
        {
            try
            {
                ...
                if (!CheckFile(filepath, filename))
                    return false;
                var sr = DataCheckProgress.GetByFile(filepath + "\\" + filename);
                var srid = sr.WKID;
                if (srid == 4490)
                {
        ...
        public string ProcessDataOnline(string url, string key)
        {
            ...
            foreach (var item in landShapeList)
            {
                ...
                ygeo = DataCheckProgress.ReprojectShape(ygeo, dreproject, sreproject, 4490);
                ...
                land.Shape = DataCheckProgress.ReprojectShape(geometry, sreproject, dreproject, srid);
