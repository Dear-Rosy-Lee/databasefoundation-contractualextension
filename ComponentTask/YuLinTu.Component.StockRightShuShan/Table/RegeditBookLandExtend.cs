...
{
    public class RegeditBookLandExtend : ExportExcelBase
    {
        ...
        {
            SetAgriLandHeadText();
            SetLineType("A1", "E" + (LandCollection.Count * 4 + 2));
        }
        ...
        private void SetAgriLandHeadText()
        {
            foreach (var item in LandCollection)
            {
                ...
                SetRange("C" + _startIndex, "C" + (_startIndex + 3), 19.50, 10, false, item.ActualArea.AreaFormat(2));
        ...
        {
            string filePath = TemplateHelper.ExcelTemplate("权证地块扩展");
