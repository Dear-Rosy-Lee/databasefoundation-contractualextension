...
namespace YuLinTu.Library.Repository
{
    ...
    /// 集体建设用地调查宗地的数据访问类
    /// </summary>
    public class MZDWRepository : RepositoryDbContext<MZDW>, IMZDWRepository
    {
          #region Ctor

        private IDataSourceSchema m_DSSchema = null;

        public MZDWRepository(IDataSource ds)
            : base(ds)
        {
        ...
        private bool CheckTableExist()
        {
            //try
            //{
            //    return m_DSSchema.AnyElement(m_DSSchema.GetElements()[0].Schema, typeof(MZDW).GetAttribute<DataTableAttribute>().TableName);
            //}
            //catch (Exception)
            //{
            //    return false;
        ...
        public List<MZDW> GetByZoneCode(string zoneCode)
        {
            ...
            object entity = (from q in DataSource.CreateQuery<MZDW>()
                             where q.ZoneCode.Equals(zoneCode)
                             orderby q.BSM
                             select q).ToList();
            return entity as List<MZDW>;
        }
        #endregion
    }
}
