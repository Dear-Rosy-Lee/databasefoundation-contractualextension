...
namespace YuLinTu.Component.ContractedLand.BoundaryCalculateTask
{
    ...
    class DKQlr
    {
            ...
            using (var shp = new ShapeFile())
            {
                ...
                for (int dkID = 0; dkID < cnt; ++dkID)
                {
                    ...
                    if (dkbm == null)
                        continue;
                    string cbfmc;
                    if (dicDkbm2Cbfmc.TryGetValue(dkbm, out cbfmc))
                    {
                        _dic[dkID] = cbfmc;
                    }
                }
        ...
        private static Dictionary<string, string> queryKeyValuePairs(IDataBase db, string tableName, string keyFieldName, string valueFieldName, string wh = null)
        {
            ...
            {
                var key = SafeConvertAux.SafeConvertToString(r[0]);
                var val = SafeConvertAux.SafeConvertToString(r[1]);
                dic[key] = val;
            }
            return dic;
        }
