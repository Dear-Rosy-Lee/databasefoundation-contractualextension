    ...
    public class TaskExportSurveyPublishExcelOperation : Task
    {
        ...
        protected override void OnGo()
        {
            ...
            try
            {
                ...
                listLand.LandNumberFormat(argument.SystemSet);  //根据系统设置进行地块编码的截取
        ...
        public bool ExportSurveyPublishExcel(TaskExportSurveyPublishExcelArgument argument, List<VirtualPerson> vps, List<ContractLand> lands)
        {
            ...
            try
            {
                ...
                foreach (VirtualPerson vp in vps)
                {
                    ...
                    var landCollectionother = lands == null ? new List<ContractLand>() : lands.FindAll(c => c.OwnerId == vp.ID && c.LandCategory != ((int)eLandCategoryType.ContractLand).ToString());
                ...
                {
                    export.SaveFilePath = openFilePath + @"\" + excelName + TemplateFile.LandInfomationExcel + ".xls";
                    ...
                    //if (argument.SystemSet.ExportTableSenderDesToVillage)
                    //{
