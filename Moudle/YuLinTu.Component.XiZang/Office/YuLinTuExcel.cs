...
namespace YuLinTu.Library.Office
{
    ...
    public abstract class ExcelBase : IDisposable
    {
        ...
        public bool Open(string fileName, int sheetindex)
        {
        ...
        /// <param name="end"></param>
        /// <param name="value"></param>
        public void InitalizeRangeImage(string start, string end, object value,int width, int height)
        {
            int startRow = InitalizeIndex(start);
            int startColumn = InitalizeLetter(start);
            int endRow = InitalizeIndex(end);
            ...
            int index = workSheet.Pictures.Add(startRow, startColumn, endRow, endColumn, value.ToString());
            Aspose.Cells.Drawing.Picture pic = workSheet.Pictures[index];
            pic.Placement = Aspose.Cells.Drawing.PlacementType.FreeFloating;
            pic.Height = Math.Abs(height);
            pic.Width = Math.Abs(width);
            pic.X += 50;
            pic.Y += 15;
            //Style s =  workSheet.CreateStyle();
            //s.HorizontalAlignment = TextAlignmentType.Center;
            //workbook.Worksheets[0].Cells[1, 1].SetStyle(s);
        ...
        public void InsertRowCell(int rowIndex, int totalRows)
        {
            if (workSheet == null)
            {
                return;
            ...
            try
            {
                workSheet.Cells.InsertRows(rowIndex, totalRows);
            }
        ...
        /// <param name="path"></param>
        /// <returns></returns>
        public bool CreateExcel(string path, int sheetindex)
        {
            return Open(path, sheetindex);
        }
