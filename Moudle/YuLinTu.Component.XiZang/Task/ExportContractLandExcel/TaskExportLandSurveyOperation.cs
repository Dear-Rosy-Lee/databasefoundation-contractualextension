...
namespace YuLinTu.Component.XiZangLZ
{
    ...
    /// 导出承包地块调查表任务类
    /// </summary>
    public class TaskExportLandSurveyOperation : Task
    {
        #region Ctor
        ...
        {
            TaskExportLandSurveyArgument argument = Argument as TaskExportLandSurveyArgument;
            if (argument == null)
            {
                return;
            }
        ...
        /// 导出数据到承包地块Word调查表
        /// </summary>
        public bool ExportLandWord(TaskExportLandSurveyArgument argument, List<YuLinTu.Library.Entity.VirtualPerson> listPerson, List<ContractLand> listLand,
            List<Dictionary> listDict, CollectivityTissue tissue)
        {
            ...
            try
            {
                ...
                foreach (YuLinTu.Library.Entity.VirtualPerson family in listPerson)
                {
                    ...
                    foreach (var land in landsOfFamily)
                    {
                        ...
                        string tempPath = TemplateHelper.ExcelTemplate("西藏农村土地承包经营权承包地块调查表");  //模板文件
                        ...
                        string filePath = openFilePath + @"\" +
                                          family.FamilyNumber.PadLeft(4, '0') + "-" + family.Name + @"\" +
                                          landNumber + "-" + TemplateFile.ContractAccountLandSurveyWord;
                ...
                result = true;
                string info = string.Format("{0}导出{1}户承包方,共{2}个承包地块Excel调查表", markDesc, listPerson.Count, indexOfLand - 1);
            ...
            {
                result = false;
                this.ReportError("导出承包地块Excel调查表失败");
                YuLinTu.Library.Log.Log.WriteException(this, "ExportLandWord(导出承包地块数据到Excel调查表)", ex.Message + ex.StackTrace);
            }
            return result;
        }
