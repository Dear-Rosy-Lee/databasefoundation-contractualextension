...
namespace YuLinTu.Component.MapFoundation
{
    ...
    /// 导入控制点图斑界面
    /// </summary>
    public partial class ImportControlPointShape : InfoPageBase
    {
        #region Ctor
        ...
        /// 构造函数
        /// </summary>
        public ImportControlPointShape(IWorkpage page, string desc = "")
        {
            InitializeComponent();
            DataContext = this;
            this.Workpage = page;
        ...
        private ImportControlPointDefine config;
        private ImportControlPointDefine importCpDefine;
        ...
        /// 界址点图斑实体配置
        /// </summary>
        public ImportControlPointDefine ImportCpDefine
        {
            get { return importCpDefine; }
            set
            {
                importCpDefine = value;
                this.ProGrid.Object = ImportCpDefine;
            }
        }
        ...
        /// 读取的控制图斑所有字段名称
        /// </summary>
        public List<KeyValue<int, string>> CpNameList { get; set; }
        ...
        private void OnLoadConfig(List<string> cpColumnNameList)
        {
            CommonConfigSelector.DisplayValueList = cpColumnNameList;
            CpNameList = CommonConfigSelector.GetUserConfigColumnInfo(cpColumnNameList.Count); //获取定义的数据源
            ProGrid.EnableObjectMetadataCache = false;
            ProGrid.Properties["index"] = CpNameList;
            ProGrid.Alert += ProGrid_Alert;
            ProGrid.Properties["importControlPointShape"] = FileName;
            systemCenter = TheApp.Current.GetSystemSettingsProfileCenter();  //系统配置
            var profile = systemCenter.GetProfile<ImportControlPointDefine>();  //得到section部分 此方法已经判断了section为空的情况，为空就用默认构造
            var section = profile.GetSection<ImportControlPointDefine>();
            config = (section.Settings as ImportControlPointDefine);   //得到经反序列化后的对象
            var configCopy = config.Clone();
            var keylist = configCopy.DictProperties();
            foreach (var item in keylist)
            {
                if (!cpColumnNameList.Contains(item.Value.ToString()))
                {
                    configCopy.SetPropertyValue(item.Key, "None");
                }
            }
            ImportCpDefine = configCopy as ImportControlPointDefine;
        }
        ...
        private void btnFileSelect_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                List<string> cpColumnNameList = new List<string>();
                ss.ForEach(t => cpColumnNameList.Add(t.ColumnName));
                CommonConfigSelector.DisplayValueList = null;
                OnLoadConfig(cpColumnNameList);
            }
            catch
            {
        ...
        private void btnExcuteImport_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                config.CopyPropertiesFrom(ImportCpDefine);
                systemCenter.Save<ImportControlPointDefine>();
            }));
        ...
        private void allCheck_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                PropertyInfo[] infos = typeof(ImportControlPointDefine).GetProperties();
                ...
                for (int i = 0; i < infos.Length; i++)
                {
                    ...
                    {
                        info.SetValue(ImportCpDefine, "None", null);
                        continue;
                    }
                    if (display == null)
                        continue;
                    ...
                    if (fvalue)
                    {
                        info.SetValue(ImportCpDefine, display.Name, null);
                    }
                }
            }
            catch
