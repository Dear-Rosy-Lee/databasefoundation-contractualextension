...
namespace YuLinTu.Library.Business
{
    ...
    /// 地域模块业务处理
    /// </summary>
    public class ExportLandCoilShapeBusiness : Task
    {
        #region Fildes
        ...
        /// 构造函数
        /// </summary>
        public ExportLandCoilShapeBusiness(IDbContext db)
        {
            this.db = db;
        ...
        /// 构造函数
        /// </summary>
        public ExportLandCoilShapeBusiness()
        {
        }
        ...
        public bool ExportZoneShape(Zone zone, string fileName)
        {
            ...
            try
            {
                ...
                IBuildLandBoundaryAddressCoilWorkStation jzxStation = DbContext.CreateBoundaryAddressCoilWorkStation();
                var LandCoilList = jzxStation.GetByZoneCode(zone.FullCode, eLevelOption.Self);
                ...
                //List<BuildLandBoundaryAddressCoil> LandCoilList = jzxStation.GetByZoneCode(zone.FullCode, eSearchOption.Precision, eLandPropertyRightType.AgricultureLand);
                List<BuildLandBoundaryAddressCoil> listGeoLandCoil = LandCoilList.FindAll(c => c.Shape != null);
                if (listGeoLandCoil == null || listGeoLandCoil.Count == 0)
                {
                    this.ReportWarn("未获取到地块界址线空间数据!");
                    this.ReportProgress(100);
                    return false;
                }
                using (var export = new ExportLandCoilGeoToShape())
                {
                    fileName = Path.Combine(fileName, markDes + "界址线.shp");
                    export.ZoneDesc = markDes;
                    export.FileName = fileName;
                    export.CurrentZone = zone;
                    export.ListGeoLandCoil = listGeoLandCoil;
            ...
            catch (Exception ex)
            {
                ...
                YuLinTu.Library.Log.Log.WriteException(this, "ExportZoneTable(导出地块界址线shape)", ex.Message + ex.StackTrace);
                reslut = false;
            }
            return reslut;
        }
