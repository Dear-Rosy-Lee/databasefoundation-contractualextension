...
namespace YuLinTu.Library.Business
{
    ...
    /// </summary>
    [Serializable]
    public class ExportSecondLandSignExcel : ExportExcelBase
    {
        #region Fields
        ...
        public ExportSecondLandSignExcel()
        {
        ...
        private bool BeginWrite()
        {
            ...
            try
            {
                ...
                foreach (var item in secondTableLandFamily)
                {
                    ...
                    SetRange("I" + index, "I" + (index + length - 1), height, 11, false, vp != null ? vp.PersonCount : "");
                    SetRange("J" + index, "J" + (index + length - 1), height, 11, false, vp != null && !string.IsNullOrEmpty(vp.TotalTableArea.ToString()) ? ToolMath.SetNumbericFormat(vp.TotalTableArea.ToString(), 2) : "");
                    if (!string.IsNullOrEmpty(vp.TotalArea))
                    {
                        totalTebleArea += ToolMath.SetNumericFormat(Convert.ToDouble(vp.TotalArea), 2, 1);
                    }
                    SetRange("R" + index, "R" + (index + length - 1), height, 11, false, "");
                    double num = 0.0;
                    ...
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        SetRange("F" + currentIndex, "F" + currentIndex, height, 11, false, person.ICN);
                        if (person.Name == item.CurrentFamily.Name && person.ICN == item.CurrentFamily.Number)
                        {
                            person.Relationship = "户主";
                        }
                        SetRange("G" + currentIndex, "G" + currentIndex, height, 11, false, person.Relationship);
                        SetRange("H" + currentIndex, "H" + currentIndex, height, 11, false, person.Comment);//备注
                        currentIndex++;
                    }
                    ...
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        #region 面积
                        SetRange("K" + currentIndex, "K" + currentIndex, height, 11, false, land.TableArea.HasValue ? ToolMath.SetNumbericFormat(land.TableArea.Value.ToString(), 2) : " ");
                        onlyTableAreaCount += land.TableArea.HasValue ? land.TableArea.Value : 0.0;
                        SetRange("L" + currentIndex, "L" + currentIndex, height, 11, false, land.LandName);
                        SetRange("M" + currentIndex, "M" + currentIndex, height, 11, false, land.Name);

                        if (SecondTableDefine.ShowSecondTableDataNeighborWithDirection)
                        {
                            SetRange("N" + currentIndex, "N" + currentIndex, height, 11, false, land.NeighborEast);//四至东
                            SetRange("O" + currentIndex, "O" + currentIndex, height, 11, false, land.NeighborSouth);//四至南
                            SetRange("P" + currentIndex, "P" + currentIndex, height, 11, false, land.NeighborWest);//四至西
                            SetRange("Q" + currentIndex, "Q" + currentIndex, height, 11, false, land.NeighborNorth);//四至北
                        }
                        else
                        {
                            SetRange("N" + currentIndex, "N" + currentIndex, height, 11, false, land.NeighborEast);//四至东
        ...
        private void WriteCount()
        {
            ...
            SetRange("I" + index, "I" + index, 28.5, 11, false, contractorCount.ToString());
            SetRange("J" + index, "J" + index, 28.5, 11, false, ToolMath.SetNumbericFormat(totalTebleArea > 0.0 ? totalTebleArea.ToString() : onlyTableAreaCount.ToString(), 2));
            SetRange("K" + index, "K" + index, 28.5, 11, false, ToolMath.SetNumbericFormat(onlyTableAreaCount.ToString(), 2));
        ...
        private void WriteTitle()
        {
            ...
            if (CurrentZone != null && CurrentZone.FullCode.Length > 0)
            {
                ...
                SetRange("A" + index, "M" + index, 22.5, 14, false, zoneStr);
            }
            else
                SetRange("A" + index, "M" + index, 22.5, 14, false, "单位:         乡镇(社区服务中心）          村        组");

            SetRange("N" + index, "S" + index, 22.5, 14, false, "日期：" + GetDate());
            if (SecondTableDefine.ShowSecondTableDataNeighborWithDirection)
            {
                SetRange("N5", "N5", "东");
                SetRange("O5", "O5", "南");
