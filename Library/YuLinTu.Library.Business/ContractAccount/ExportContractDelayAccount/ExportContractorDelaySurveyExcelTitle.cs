...
namespace YuLinTu.Library.Business
{
    ...
    public partial class ExportContractorDelaySurveyExcel
    {
        ...
        {
            string titleName = "第二轮土地承包到期后再延长30年确权台账表";

            if (!string.IsNullOrEmpty(titleName))
            {
                SetRange("A1", "A1", 32.25, 18, true, titleName);
            }
            InitalizeRangeValue("A3", "A5", "编号");
            SetRange("A3", "A5", "编号");
            InitalizeRangeValue("B3", "B5", "户主名称");
            SetRange("B3", "B5", "户主名称");
            SetRange("I2", "M2", 21.75, 11, false, 4, 2, "日期:" + GetDate() + "               ");
            SetRange("A2", "H2", "单位:  " + ExcelName);
            columnIndex = 2;
            ...
            //{
            //    columnIndex++;
            //    SetRange(PublicityConfirmDefine.GetColumnValue(columnIndex) + 4, PublicityConfirmDefine.GetColumnValue(columnIndex) + 5, "签字人(按印)", true);
            //}
        }
        ...
        private int WriteContractorTitle(int columnIndex)
        {
            ...
            if (columnIndex > 2)
            {
                SetRange("C3", "H3", "家庭成员情况（含户主）", true);
            }
            return columnIndex;
        }
        ...
        private int WriterCensusTitle(int columnIndex)
        {
            ...
            {
                SetRange(PublicityConfirmDefine.GetColumnValue(startIndex) + 4, PublicityConfirmDefine.GetColumnValue(columnIndex) + 4, "户籍籍贯信息", true);
            }
            return columnIndex;
        }
        ...
        private int WrireSecondContractorTitle(int columnIndex)
        {
            ...
            if ((columnIndex - startIndex) >= 0)
            {
                SetRange(PublicityConfirmDefine.GetColumnValue(startIndex) + 4, PublicityConfirmDefine.GetColumnValue(columnIndex) + 4, contractLandOutputSurveyDefine.ExPackageNameValue ? "二轮土地延包人口和现有家庭人口变动情况" : "二轮承包家庭成员情况（含户主）", true);
            }
            startIndex = 0;
            return columnIndex;
        }
        ...
        private int WriterAgricultureLandTitle(int columnIndex)
        {
            ...
            {
                columnIndex++;
                SetRange(PublicityConfirmDefine.GetColumnValue(columnIndex) + 4, PublicityConfirmDefine.GetColumnValue(columnIndex) + 5, "宗地数量", true);
            }
            ...
            if (contractLandOutputSurveyDefine.LandNeighborValue)
            {
                ...
                SetRange(PublicityConfirmDefine.GetColumnValue(columnIndex - 4) + 4, PublicityConfirmDefine.GetColumnValue(columnIndex) + 4, "四至", true);
            }
            ...
            if (columnIndex > 2)
            {
                SetRange("I3", "M3", "农村土地承包经营权汇总情况", true);
            }
            return columnIndex;
        }
        ...
        private int WriteAgricultureLandExpandTitle(int columnIndex)
        {
            ...
            {
                SetRange(PublicityConfirmDefine.GetColumnValue(startIndex) + 4, PublicityConfirmDefine.GetColumnValue(columnIndex) + 4, "承包地流转情况", true);
            }
            ...
            if ((columnIndex - startIndex) >= 0)
            {
                SetRange(PublicityConfirmDefine.GetColumnValue(startIndex) + 4, PublicityConfirmDefine.GetColumnValue(columnIndex) + 4, "承包地收益情况", true);
            }
            return columnIndex;
        }
        ...
        private int WriteSecondLandTitle(int columnIndex)
        {
            ...
            if ((columnIndex - startIndex) >= 0)
            {
                SetRange(PublicityConfirmDefine.GetColumnValue(startIndex) + 4, PublicityConfirmDefine.GetColumnValue(columnIndex) + 4, "二轮农村土地承包情况", true);
            }
            return columnIndex;
        }
