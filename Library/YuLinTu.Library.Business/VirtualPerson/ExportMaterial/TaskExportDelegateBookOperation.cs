...
namespace YuLinTu.Library.Business
{
    ...
    /// 导出委托声明书任务类
    /// </summary>
    public class TaskExportDelegateBookOperation : Task
    {
        #region Ctor
        ...
        /// 构造函数
        /// </summary>
        public TaskExportDelegateBookOperation()
        { }
        ...
        {
            TaskExportDelegateBookArgument argument = Argument as TaskExportDelegateBookArgument;
            if (argument == null)
            {
                return;
            }
            ...
            try
            {
                ...
                bool canOpen = ExportDelegateBookWord(argument, listPerson);
                if (canOpen)
                    CanOpenResult = true;
            }
            catch (Exception ex)
            {
                YuLinTu.Library.Log.Log.WriteException(this, "TaskExportDelegateBookOperation(导出委托声明书任务)", ex.Message + ex.StackTrace);
                this.ReportException(ex, "导出委托声明书出现异常!");
            }
        }
        ...
        /// 导出数据到委托声明书
        /// </summary>
        public bool ExportDelegateBookWord(TaskExportDelegateBookArgument argument, List<VirtualPerson> listPerson)
        {
            bool result = false;
            try
            {
                ...
                foreach (VirtualPerson family in listPerson)
                {
                    ...
                    exportFamily.DateValue = argument.Time == null ? DateTime.Now : argument.Time;
                    ...
                    exportFamily.RightName = InitalizeRightType(argument.VirtualType);
                    exportFamily.SaveAs(family, openFilePath + @"\" + familyNuber + "-" + family.Name + "-" + TemplateFile.VirtualPersonDelegateBook + ".doc");
                    this.ReportProgress((int)(1 + vpPercent * index), markDesc + family.Name);
                    index++;
                }
                result = true;
                string info = string.Format("{0}导出{1}张委托声明书", markDesc, index - 1);
            ...
            {
                result = false;
                this.ReportError("导出委托声明书失败");
                YuLinTu.Library.Log.Log.WriteException(this, "ExportDelegateBookWord(委托声明书处理)", ex.Message + ex.StackTrace);
            }
            return result;
        }
